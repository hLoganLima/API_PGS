...
def fetch_cliente_data_row_by_row(supabase_url, supabase_key):
    """
    Busca registros da tabela 'cliente' do Supabase um por um e os salva em um JSON na pasta Data.
    """
    # Conecta ao Supabase
    ...
    row_index = 0
    while True:
        # Busca um registro por vez da tabela 'cliente'
        try:
            response = supabase_client.table("cliente").select("*").range(row_index, row_index).execute()
            ...
            if response.data:
                ...
                print("Todos os registros da tabela 'cliente' foram processados.")
                break
    ...
    if all_data:
        ...
        json_path = os.path.join(data_dir, "data_cliente_supabase.json")

        # Salva os dados em um arquivo JSON na pasta 'Data'
        with open(json_path, 'w') as temp_file:
            json.dump(all_data, temp_file, indent=4)
        print(f"Todos os dados da tabela 'cliente' salvos em: {json_path}")
        return json_path
    else:
        print("Nenhum dado encontrado na tabela 'cliente'.")
        return None


# Teste: Lendo dados da tabela 'cliente' registro por registro e salvando no JSON
if __name__ == "__main__":
    json_file_path = fetch_cliente_data_row_by_row(supabase_url, supabase_key)
    if json_file_path:
